...
*Describe, en términos morfométricos, el límite de tu cuenca y abunda sobre ella. Algunas preguntas guía: ¿Qué forma tiene? ¿A qué atribuyes su forrñma? ¿Es importante esta cuenca? ¿Por qué?*

La cuenca del rio bao en la localidad de Constanza tiene forma como de un heptagono parecido al país serbia, su forma se le puede atribuir a la topografia del terreno.
...
execGRASS(
 ...
 plot(red80)
 plot(red30)
 plot(red80); par(mfrow=c(1,2))
 plot(red80)
 plot(red30)

red8011 <- spTransform(red80, CRSobj = CRS("+init=epsg:4326"))
red8011
red38011 <- spTransform(red80, CRSobj = CRS("+init=epsg:4326"))
red38011

library(leaflet)
leaflet() %>% 
  addPolylines(data = red8011, weight = 3, opacity = 0.7, group = 'str_vect') %>% 
  leafem::addHomeButton(extent(riobao_net4326), 'Ver todo') %>% 
  addLayersControl(
    overlayGroups = c('terrain','str_vect','str_raster'),
    options = layersControlOptions(collapsed=FALSE)) 

leaflet() %>% 
  addPolylines(data = red3011, weight = 3, opacity = 0.7, group = 'str_vect') %>% 
